	...
	switch (slot_state)
	{
		...
		case DMR_STATE_TX_START_4: // Start TX (fifth step)
			...
			if (settingsUsbMode != USB_MODE_HOTSPOT)
			{
				...
				// ClosedGD77: Initialize encryption for new TX call
				encryption_reset_per_call();
				encryption_superframe_begin();
			}
			break;
		...
		else
		{
			...
			else
			{
				...
				if (wavbuffer_count >= 2)
				{
					...
					// ClosedGD77: Encrypt AMBE voice block before buffering
					if (encryption_is_enabled())
					{
						encryption_encrypt_voice_block((uint8_t *)deferredUpdateBufferInPtr, LENGTH_AMBE_BLOCK);
					}
	...
	else
	{
		...
		else
		{
			...
			if (!voicePromptsIsPlaying() && hasEncodedAudio)
			{
				...
				// ClosedGD77: Initialize RX decryption on first voice frame of call
				if (encryption_is_enabled() && slot_state < DMR_STATE_TX_START_1)
				{
					if (!encryption_get_state()->rx_keystream_ready)
					{
						encryption_superframe_begin();
					}
				}

				// ClosedGD77: Decrypt AMBE voice frame before codec decode
				if (encryption_is_enabled())
				{
					encryption_decrypt_voice_frame((uint8_t *)DMR_frame_buffer + 0x0C, 27);
				}
