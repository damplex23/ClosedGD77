...
void encryption_set_global_mode(uint8_t mode)
{
    if (mode == 0xFF)
    {
        // Use codeplug — flag already stored, apply on next channel change
    ...
    {
        enc_state.enabled = false;       // not DMR encryption
...
void encryption_scramble_audio(int16_t *samples, uint16_t count)
{
    ...
    // ponytail: pass scramble_id (1-8) for carrier frequency selection, 0 for default 3.2kHz
    uint8_t id = enc_state.scramble_id ? enc_state.scramble_id : 4; // default ID 4 = 3.2kHz
