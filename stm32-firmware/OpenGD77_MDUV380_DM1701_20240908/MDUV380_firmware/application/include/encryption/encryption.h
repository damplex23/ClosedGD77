...
typedef struct {
    ...
    uint8_t  scramble_id;                // 0=off, 1-8=carrier frequency index
    uint16_t scramble_carrier_phase;     // phase accumulator for scrambler
    uint8_t  global_mode_override;       // 0xFF=use codeplug, 0=Off, 1=ARC4, 2=AES, 4=Scrambler
} encryption_state_t;
...
// Analog scrambler ID cycling — 0=off, 1-8=carrier frequency index
void encryption_set_scramble_id(uint8_t id);
uint8_t encryption_get_scramble_id(void);

// Global encryption mode override (menu-driven) — 0xFF=use codeplug
